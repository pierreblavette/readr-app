...
const READR_READING_GOAL_KEY = 'readr-reading-goal';

export const MAX_READING = 3;
export const MAX_BOOKS_PER_COLLECTION = 10;
...
export function useLibrary() {
  ...
  const [readingGoal, setReadingGoalState]      = useState(null);
  ...
  useEffect(() => {
    try {
      ...
      if (['overview', 'owned', 'wishlist', 'collections', 'quotes', 'dictionary'].includes(savedTab)) setTabState(savedTab);
      ...
      const savedGoal = localStorage.getItem(READR_READING_GOAL_KEY);
      if (savedGoal) {
        try {
          const g = JSON.parse(savedGoal);
          if (g && typeof g.year === 'number' && typeof g.count === 'number') {
            setReadingGoalState(g);
          }
        } catch(e) {}
      }
    } catch(e) {}
  ...
  // Auto-collapse de la sidebar sous 960px (zone desktop étroite) ; au-dessus on
  // restaure la préférence manuelle persistée. Le drawer mobile prend le relais ≤768.
  useEffect(() => {
    const mq = window.matchMedia('(max-width: 960px)');
    const apply = () => {
      if (mq.matches) setSidebarCollapsed(true);
      else setSidebarCollapsed(localStorage.getItem(READR_SIDEBAR_KEY) === '1');
    };
    apply();
  ...
  const setReadingGoal = useCallback((year, count) => {
    const next = (year && count > 0) ? { year, count } : null;
    setReadingGoalState(next);
    try {
      if (next) localStorage.setItem(READR_READING_GOAL_KEY, JSON.stringify(next));
      else localStorage.removeItem(READR_READING_GOAL_KEY);
    } catch(e) {}
  }, []);
  ...
  // sortCol === 'dateFinished' is a filter+sort: only finished books, by finishedAt desc.
  // sortCol === 'dateAdded' on owned tab uses addedToLibraryAt || id so moves from wishlist
  // are surfaced as recent (id is the original wishlist-add timestamp for moved books).
  const books = (() => {
    let list = [...(data[tab] || [])];
    if (sortCol === 'dateFinished') {
      list = list.filter(b => b.finishedAt);
    }
    // Filters are owned-tab only (rating/reading status/quotes don't apply elsewhere).
    if (tab === 'owned') {
      if (filters.hasQuotes) {
        list = list.filter(b => quotes.some(q => q.bookId === b.id));
      }
      if (filters.readingStatus !== 'any') {
        list = list.filter(b => {
          if (filters.readingStatus === 'notStarted') return !b.startedAt;
      ...
      if (filters.genres.size > 0) {
        list = list.filter(b => filters.genres.has(b.genre || ''));
      }
      if (filters.authors.size > 0) {
        list = list.filter(b => filters.authors.has((b.author || '').trim()));
      }
    }
    if (search.trim()) {
    ...
    list.sort((a, b) => {
      ...
      if (sortCol === 'dateAdded') {
        const aA = (tab === 'owned' ? (a.addedToLibraryAt || a.id) : a.id) || 0;
        const bA = (tab === 'owned' ? (b.addedToLibraryAt || b.id) : b.id) || 0;
        cmp = aA - bA;
      } else if (sortCol === 'dateFinished') {
  ...
  // Book-mutation tabs are 'owned' and 'wishlist'. Other tabs (overview,
  // collections, quotes, dictionary) are non-book contexts — when an add/
  // delete is triggered from them (e.g. CTA "Add a book" on Overview),
  // fall back to 'owned'.
  const bookTab = (tab === 'owned' || tab === 'wishlist') ? tab : 'owned';

  const addBook = useCallback((book, options = {}) => {
    const isDuplicate = data[bookTab].some(
    ...
    const id = Date.now();
    let newBook = { ...book, id };
    // Atomic "add + mark as reading" path: avoid the state race between
    // addBook() and a separate startReading() call (which would close over
    // a stale `data` snapshot and silently no-op).
    if (options.startReading && bookTab === 'owned') {
      const currentlyReading = data.owned.filter(b => b.startedAt && !b.finishedAt).length;
      if (currentlyReading < MAX_READING) {
        newBook = { ...newBook, startedAt: Date.now(), finishedAt: null };
      }
    }
    const newData = { ...data, [bookTab]: [...data[bookTab], newBook] };
    saveData(newData);
    return id;
  }, [data, bookTab, saveData]);

  const addMany = useCallback((books) => {
    const existing = data[bookTab];
    ...
    const newData = { ...data, [bookTab]: [...existing, ...toAdd] };
    saveData(newData);
  }, [data, bookTab, saveData]);

  const deleteBook = useCallback((id) => {
    const newData = { ...data, [bookTab]: data[bookTab].filter(b => b.id !== id) };
    saveData(newData);
    if (panelBook?.id === id) setPanelBook(null);
  }, [data, bookTab, saveData, panelBook]);

  const moveToLibrary = useCallback((ids) => {
    const now = Date.now();
    const toMove = data.wishlist
      .filter(b => ids.has(b.id))
      .map(b => ({ ...b, addedToLibraryAt: now }));
    const newData = {
  ...
  const deleteMany = useCallback((ids) => {
    const newData = { ...data, [bookTab]: data[bookTab].filter(b => !ids.has(b.id)) };
    saveData(newData);
  }, [data, bookTab, saveData]);
  ...
  const exportPDF = useCallback(() => {
    ...
    const lines = [
      ...
      '.logo{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:28px;font-weight:700;color:#111827;letter-spacing:-0.5px}',
  ...
  const createCollection = useCallback((name) => {
    const col = { id: Date.now(), name, bookIds: [] };
    saveCollections([...collections, col]);
    return col;
  }, [collections, saveCollections]);

  const createCollectionWithBook = useCallback((name, bookId) => {
    const col = { id: Date.now(), name, bookIds: bookId ? [bookId] : [] };
    saveCollections([...collections, col]);
    return col;
  }, [collections, saveCollections]);
  ...
  const deleteCollections = useCallback((ids) => {
    const set = ids instanceof Set ? ids : new Set(ids);
    saveCollections(collections.filter(c => !set.has(c.id)));
    if (set.has(activeCollection)) setActiveCollection(null);
  }, [collections, activeCollection, saveCollections]);
  ...
  const addBookToCollection = useCallback((colId, bookId) => {
    ...
    if (!col) return false;
    const ids = col.bookIds || [];
    if (ids.includes(bookId)) return true;
    if (ids.length >= MAX_BOOKS_PER_COLLECTION) return false;
    saveCollections(collections.map(c =>
      c.id === colId ? { ...c, bookIds: [...ids, bookId] } : c
    ));
    return true;
  }, [collections, saveCollections]);

  const addBooksToCollection = useCallback((colId, bookIds) => {
    const col = collections.find(c => c.id === colId);
    if (!col) return 0;
    const existing = col.bookIds || [];
    const room = MAX_BOOKS_PER_COLLECTION - existing.length;
    if (room <= 0) return 0;
    const toAdd = bookIds.filter(id => !existing.includes(id)).slice(0, room);
    if (toAdd.length === 0) return 0;
    saveCollections(collections.map(c =>
      c.id === colId ? { ...c, bookIds: [...existing, ...toAdd] } : c
    ));
    return toAdd.length;
  }, [collections, saveCollections]);
  ...
  const removeBooksFromCollection = useCallback((colId, bookIds) => {
    const set = bookIds instanceof Set ? bookIds : new Set(bookIds);
    saveCollections(collections.map(c =>
      c.id === colId ? { ...c, bookIds: (c.bookIds || []).filter(id => !set.has(id)) } : c
    ));
  }, [collections, saveCollections]);
  ...
  const addQuote = useCallback(({ text, bookTitle, bookAuthor, bookId, page, saved }) => {
    const quote = {
      id: Date.now(),
      text: text.trim(),
  ...
  const updateQuote = useCallback((id, updates) => {
    saveQuotes(quotes.map(q => q.id === id ? {
      ...
      saved: updates.saved !== undefined ? !!updates.saved : q.saved,
    } : q));
  }, [quotes, saveQuotes]);
  ...
  const exportWordsPDF = useCallback(() => {
    ...
    const lines = [
      ...
      'h3{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:20px;font-weight:700;color:#111827;margin-bottom:12px}',
  ...
  const exportQuotesPDF = useCallback(() => {
    ...
    const lines = [
      ...
      'h3{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:18px;font-weight:700;color:#111827}',
  ...
  // -------- Collections export -----------------------------------------
  // Helpers : booksFor + filename slug shared between MD and PDF.
  const collectionsBooksFor = useCallback((col) => {
    const all = [...(data.owned || []), ...(data.wishlist || [])];
    return (col.bookIds || []).map(id => all.find(b => b.id === id)).filter(Boolean);
  }, [data]);
  ...
  const exportCollectionsMD = useCallback(() => {
    const isFr = lang === 'fr';
    const date = new Date().toLocaleDateString(isFr ? 'fr-FR' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    const title = isFr ? 'Collections' : 'Collections';
    const colCountLabel = isFr
      ? (collections.length > 1 ? 'collections' : 'collection')
      : (collections.length > 1 ? 'collections' : 'collection');
    const bookCountLabel = (n) => isFr ? (n > 1 ? 'livres' : 'livre') : (n > 1 ? 'books' : 'book');
    const emptyLine = isFr ? '_Aucune collection._' : '_No collections._';
    const emptyColLine = isFr ? '_Aucun livre._' : '_No books._';

    const lines = [`# readr — ${title}`, `_${date} · ${collections.length} ${colCountLabel}_`, ''];
    if (!collections.length) {
      lines.push(emptyLine);
    } else {
      collections.forEach(col => {
        const books = collectionsBooksFor(col);
        lines.push(`## ${col.name}`);
        lines.push(`_${books.length} ${bookCountLabel(books.length)}_`, '');
        if (!books.length) {
          lines.push(emptyColLine, '');
        } else {
          books.forEach(b => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'readr-collections.md'; a.click();
    URL.revokeObjectURL(url);
  }, [collections, lang, collectionsBooksFor]);

  const exportCollectionsPDF = useCallback(() => {
    function esc(str) {
      return String(str || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    }
    const isFr = lang === 'fr';
    ...
    const colCountLabel = isFr
      ...
      : (collections.length > 1 ? 'collections' : 'collection');
    const bookCountLabel = (n) => isFr ? (n > 1 ? 'livres' : 'livre') : (n > 1 ? 'books' : 'book');
    const emptyStr = isFr ? 'Aucune collection' : 'No collections';
    const emptyColStr = isFr ? 'Aucun livre' : 'No books';
    const thTitle = isFr ? 'Titre' : 'Title';
    const thAuthor = isFr ? 'Auteur' : 'Author';
    const thYear = isFr ? 'Année' : 'Year';
    ...
    function renderCollection(col) {
      ...
      const rows = books.length
        ? `<table><thead><tr><th>${thTitle}</th><th>${thAuthor}</th><th>${thYear}</th><th>Genre</th></tr></thead>
          <tbody>${books.map(b => `<tr>
            <td class="td-title">${esc(b.title)}</td>
            <td class="td-author">${esc(b.author)}</td>
            <td class="td-meta">${esc(b.year) || '—'}</td>
        ...
        : `<p class="empty">${emptyColStr}</p>`;
      return `<article class="entry">
        <header class="col-head">
          <h3>${esc(col.name)}</h3>
          <span class="col-count">${books.length} ${bookCountLabel(books.length)}</span>
        </header>
        ${rows}
      </article>`;
    }
    ...
    const lines = [
      ...
      '.meta{font-size:11px;color:#9ca3af}',
      'h2{font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:0.1em;color:#4959E6;margin-bottom:16px}',
      '.entry{padding:20px 0;border-bottom:1px solid #f3f4f6;break-inside:avoid}',
      '.entry:last-child{border-bottom:none}',
      '.col-head{display:flex;align-items:baseline;justify-content:space-between;gap:12px;margin-bottom:12px}',
      'h3{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:18px;font-weight:700;color:#111827}',
      '.col-count{font-size:11px;color:#9ca3af}',
      'table{width:100%;border-collapse:collapse}',
      'th{text-align:left;font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:0.08em;color:#9ca3af;padding:8px 12px;border-bottom:1px solid #e5e7eb}',
      'td{padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;vertical-align:top}',
      '.td-title{color:#111827;font-weight:500}',
      '.td-author{color:#4b5563}',
      '.td-meta{color:#9ca3af}',
      ...
      '<div class="header">',
      '<span class="logo">readr</span>',
      `<span class="meta">${date} · ${collections.length} ${colCountLabel}</span>`,
      '</div>',
      `<h2>${title}</h2>`,
      collections.length ? collections.map(renderCollection).join('') : `<p class="empty">${emptyStr}</p>`,
      '</body>',
      '</html>',
    ];
  ...
  const exportCollectionMD = useCallback((colId) => {
    const col = collections.find(c => c.id === colId);
    if (!col) return;
    const isFr = lang === 'fr';
    const date = new Date().toLocaleDateString(isFr ? 'fr-FR' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    const books = collectionsBooksFor(col);
    const bookCountLabel = isFr ? (books.length > 1 ? 'livres' : 'livre') : (books.length > 1 ? 'books' : 'book');
    const emptyLine = isFr ? '_Aucun livre._' : '_No books._';

    const lines = [`# readr — ${col.name}`, `_${date} · ${books.length} ${bookCountLabel}_`, ''];
    if (!books.length) {
      lines.push(emptyLine);
    } else {
      books.forEach(b => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `readr-${slugFilename(col.name)}.md`; a.click();
    URL.revokeObjectURL(url);
  }, [collections, lang, collectionsBooksFor]);

  const exportCollectionPDF = useCallback((colId) => {
    const col = collections.find(c => c.id === colId);
    if (!col) return;
    function esc(str) {
      return String(str || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    ...
    const lines = [
      ...
      '<head>',
      '<meta charset="utf-8">',
      `<title>readr — ${esc(col.name)}</title>`,
      '<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">',
      '<style>',
      '*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}',
      'body{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:12px;color:#111827;background:#fff;padding:48px 56px}',
      ...
      '.logo{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:28px;font-weight:700;color:#111827;letter-spacing:-0.5px}',
      '.meta{font-size:11px;color:#9ca3af}',
      'h2{font-family:"Plus Jakarta Sans",system-ui,sans-serif;font-size:24px;font-weight:700;color:#111827;margin-bottom:16px}',
      'table{width:100%;border-collapse:collapse}',
      'th{text-align:left;font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:0.08em;color:#9ca3af;padding:8px 12px;border-bottom:1px solid #e5e7eb}',
      'td{padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;vertical-align:top}',
      '.td-title{color:#111827;font-weight:500}',
      ...
      '<div class="header">',
      '<span class="logo">readr</span>',
      `<span class="meta">${date} · ${books.length} ${bookCountLabel}</span>`,
      '</div>',
      `<h2>${esc(col.name)}</h2>`,
      table,
      '</body>',
