...
export function useLibrary() {
  ...
  // Recent sub-lists (90-day window, search-filtered, "most-recent-first" —
  // sort/sortCol ignored). Owned tab: books with finishedAt in last 90 days.
  // Wishlist tab: books added in last 90 days (id is a timestamp).
  const RECENT_WINDOW_DAYS = 90;
  const recentlyFinished = (() => {
    if (tab !== 'owned') return [];
    const cutoff = Date.now() - RECENT_WINDOW_DAYS * 24 * 60 * 60 * 1000;
    let list = (data.owned || []).filter(b => b.finishedAt && b.finishedAt >= cutoff);
    if (search.trim()) {
      const q = search.trim().toLowerCase();
      list = list.filter(b =>
        b.title.toLowerCase().includes(q) || b.author.toLowerCase().includes(q)
  ...
  const recentlyAdded = (() => {
    ...
    const cutoff = Date.now() - RECENT_WINDOW_DAYS * 24 * 60 * 60 * 1000;
    let list = (data.wishlist || []).filter(b => b.id && b.id >= cutoff);
    if (search.trim()) {
      const q = search.trim().toLowerCase();
      list = list.filter(b =>
        b.title.toLowerCase().includes(q) || b.author.toLowerCase().includes(q)
  ...
  return {
    ...
    books, recentlyFinished, recentlyAdded, hydrated,
