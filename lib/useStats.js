...
export function useStats({ owned = [], quotes = [], words = [], readingGoal = null } = {}) {
  return useMemo(() => {
    ...
    const mostLoved = finishedBooks
      .filter(b => typeof b.rating === 'number' && b.rating >= 4)
      .sort((a, b) => {
    ...
    // dayOfYear = 1..365/366. Projected = floor(progress * 365 / dayOfYear).
    const yearStart = new Date(currentYear, 0, 1).getTime();
    const dayOfYear = Math.max(1, Math.floor((Date.now() - yearStart) / MS_PER_DAY) + 1);
    const yearLength = ((currentYear % 4 === 0 && currentYear % 100 !== 0) || currentYear % 400 === 0) ? 366 : 365;
    const projected = Math.floor((finishedThisYear * yearLength) / dayOfYear);
    goalState.pace = goalState.isSet ? {
      projected,
      delta: projected - goalState.target,
    } : null;

    const finishedSorted = [...finishedBooks].sort(
