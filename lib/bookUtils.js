...
// Set module-level des URLs de cover déjà décodées au moins une fois dans la
// session. Survit aux remontages de composants (changement d'onglet) : une carte
// qui se remonte lit ce Set en sync et initialise imageLoaded=true → pas de
// re-flash du skeleton pour une image déjà affichée. Complète liveImageRefs
// (qui garde les octets au chaud) côté état React.
const decodedCovers = new Set();
export function isCoverDecoded(url) { return !!url && decodedCovers.has(url); }
export function markCoverDecoded(url) { if (url) decodedCovers.add(url); }
...
/**
 * Build marketplace search URLs for a book — used by the BookPanel "Find online"
 * section on wishlist items. Fall back gracefully on empty title/author.
 */
function bookQuery(book) {
  return `${book?.title || ''} ${book?.author || ''}`.trim();
}

export function getAmazonSearchUrl(book) {
  const query = bookQuery(book);
  if (!query) return 'https://www.amazon.fr/';
  const params = new URLSearchParams({ k: query, i: 'stripbooks' });
  return `https://www.amazon.fr/s?${params.toString()}`;
}

export function getFnacSearchUrl(book) {
  const query = bookQuery(book);
  if (!query) return 'https://www.fnac.com/';
  const params = new URLSearchParams({ Search: query, SCat: '2' });
  return `https://www.fnac.com/SearchResult/ResultList.aspx?${params.toString()}`;
}
