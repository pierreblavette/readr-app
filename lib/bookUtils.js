...
export async function fetchBookCover(title, author, cache) {
  ...
  try {
    ...
    const url = `https://www.googleapis.com/books/v1/volumes?q=${q}&maxResults=1&fields=items(volumeInfo(imageLinks,publishedDate))`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(res.status);
    const data = await res.json();
    const item = data.items?.[0]?.volumeInfo;
    const thumb = item?.imageLinks?.thumbnail?.replace('http:', 'https:') || null;
    return { thumb, year: item?.publishedDate?.slice(0, 4) || null };
  } catch(e) {
