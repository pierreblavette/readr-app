    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal add-to-col-modal" ref={modalRef} tabIndex={-1} role="dialog" aria-modal="true">
              ...
              {sortedCollections.map(col => {
                ...
                return (
                  <button
                    key={col.id}
                    type="button"
                    className={`book-row book-row-interactive collection-chip${alreadyIn ? ' is-already-in' : ''}`}
                    disabled={alreadyIn}
        ...
        <div className="modal-actions">
          <button className="btn btn-primary btn-md" onClick={handleCreate} style={{ marginLeft: 'auto' }}>
